...
public class Goliath : MonoBehaviour, EnemyInterface
{
    ...
    void Awake()
    {
        ...
        LeftLeg1 = Instantiate(Legs[0], transform.position + VectorAdd(-0.4583f, -0.4375f, 3) * size, Quaternion.identity); // ???? ??? ??リ?
        LeftLeg2 = Instantiate(Legs[0], transform.position + VectorAdd(-0.2916f, -0.4375f, 1) * size, Quaternion.identity); // ???? ??? ??リ?
        RightLeg1 = Instantiate(Legs[1], transform.position + VectorAdd(0.2916f, -0.4375f, 3) * size, Quaternion.identity); // ?????? ??? ??リ?
        RightLeg2 = Instantiate(Legs[1], transform.position + VectorAdd(0.4583f, -0.4375f, 1) * size, Quaternion.identity); // ?????? ??? ??リ?
